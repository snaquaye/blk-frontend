...
export function getStrapiImageUrl(image?: any): string | undefined {
  ...
  // Strapi sometimes returns media fields as arrays (even for single
  // images). If that's the case, just process the first element.
  if (Array.isArray(image)) {
    if (image.length === 0) return undefined;
    return getStrapiImageUrl(image[0]);
  }
  ...
  if (typeof image === "string") {
    ...
    urlString =
      typeof image.url === "string" ? image.url : image.url.toString();
...
/**
 * Get cover image URL from article - checks multiple sources
 * Priority: coverImage (media field) > coverImageUrl (string field)
 */
export function getArticleImageUrl(article: any): string | undefined {
  // First try coverImage (media field)
  if (article.coverImage) {
    const url = getStrapiImageUrl(article.coverImage);
    if (url) return url;
  }

  // Fall back to coverImageUrl (string field for external URLs)
...
export async function getArticlesByTags(
  ...
  const tagFilter = tagSlugs
    .map(
      (slug, i) => `filters[tags][slug][$in][${i}]=${encodeURIComponent(slug)}`,
    )
    .join("&");
